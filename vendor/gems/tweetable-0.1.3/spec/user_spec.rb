...
describe Tweetable::User do 
  ...
  context "when updating info" do
    before do
      stub_get('/users/show/flippyhead.json', 'flippyhead.json', {:login => 'l', :password => 'p'})
    end
  ...
  context 'when updating friends and followers' do
    before do 
      stub_get('/followers/ids.json?screen_name=flippyhead&page=1', 'follower_ids.json', {:login => 'l', :password => 'p'})
      stub_get('/friends/ids.json?screen_name=flippyhead&page=1', 'friend_ids.json', {:login => 'l', :password => 'p'})
    end
